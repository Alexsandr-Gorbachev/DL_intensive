...
def получить_данные(акции, количество_анализируемых_дней, период_предсказания):
  if not os.path.exists('инструменты/трейдинг'):
      os.makedirs('инструменты/трейдинг')
  акции = акции.lower()
  if акции=='газпром':
    f = 'трейдинг/GAZP_1d_from_MOEX.txt'
  elif акции =='яндекс':
    f = 'трейдинг/YNDX_1d_from_MOEX.txt'
  elif акции =='полиметаллы':
    f = 'трейдинг/POLY_1d_from_MOEX.txt'
  else:
  ...
  pickle.dump(инструменты, open('инструменты/трейдинг/scaler.pkl','wb'))
  data.to_csv('инструменты/трейдинг/data.csv')
  return (x1,y1), (x2,y2)
...
def show_data(начало=0, конец=0):
    assert os.path.exists(f'инструменты/трейдинг/data.csv'), f'Загрузите базу трейдинг'
    data = pd.read_csv(f'инструменты/трейдинг/data.csv')
...
def model_test(model,
 ...
 количество_анализируемых_дней
    ...
    global price
    global real_price
    скеллер = pickle.load(open('инструменты/трейдинг/scaler.pkl','rb'))
...
def trading(model, тестовая_выборка):
  x_test = тестовая_выборка
  xScaler = pickle.load(open('инструменты/трейдинг/scaler.pkl','rb'))
