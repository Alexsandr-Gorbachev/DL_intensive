...
def get_images(**kwargs):
  ...
  if 'размер' not in kwargs:
      kwargs['размер'] = (80, 160)
  if kwargs['путь'] in data_list:
    all_paths = ['original', 'segment']
    all_airs = [[],[]]
    for idx,name in enumerate(all_paths):
        curr_path = os.path.join(kwargs['путь'], name) 
...
def show_sample(**kwargs):
  if 'количество' in kwargs:
    n = kwargs['количество'] # Количество выводимых случайных картинок
     ...
     n = 5
  fig, axs = plt.subplots(2, n, figsize=(25, 6)) #Создаем полотно из n графиков
  # Выводим в цикле n случайных изображений
  for i in range(n):
    idx = np.random.choice(os.listdir(kwargs['оригиналы']))
    img = Image.open(f"{kwargs['оригиналы']}{idx}")
    axs[0,i].imshow(img) # Отображаем фото
    axs[0,i].axis('off')  # Отключаем оси   
    if os.path.exists(f"{kwargs['сегментированные_изображения']}{idx}"):
      img2 = Image.open(f"{kwargs['сегментированные_изображения']}{idx}")
    elif os.path.exists(f"{kwargs['сегментированные_изображения']}{idx}".replace('jpg','png')):
      img2 = Image.open(f"{kwargs['сегментированные_изображения']}{idx}".replace('jpg','png'))
